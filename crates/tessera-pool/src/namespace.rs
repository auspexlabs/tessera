...
pub struct NamespaceHandle {
    ...
    digest_prefix: [u8; 16],
    /// Full 32-byte BLAKE3 digest. Stored in the SHM header so attachers
    /// can cross-verify their description against the creator's.
    full_digest: [u8; 32],
}
...
impl NamespaceHandle {
    ...
    /// POSIX SHM region name (`/tessera-pool-<hex>`).
    pub fn shm_name(&self) -> String {
        let mut out = String::from("/tessera-pool-");
...
mod tests {
    ...
    fn same_description_derives_same_handle() {
        let a = NamespaceHandle::derive("my-app/training-batches");
        let b = NamespaceHandle::derive("my-app/training-batches");
    ...
    fn different_descriptions_derive_different_handles() {
        let a = NamespaceHandle::derive("my-app/training-batches");
        let b = NamespaceHandle::derive("my-app/scoring-batches");
    ...
    fn shm_name_has_expected_shape() {
        ...
        let name = h.shm_name();
        // "/tessera-pool-" + 32 hex chars
        assert!(name.starts_with("/tessera-pool-"));
        assert_eq!(name.len(), "/tessera-pool-".len() + 32);
        let hex = &name["/tessera-pool-".len()..];
    ...
    fn empty_description_is_handled() {
        ...
        // BLAKE3(empty) starts with af13... — well-known constant.
        // We don't pin the exact bytes here (would be a fragile test);
        // just confirm derivation doesn't panic.
        let _ = h.shm_name();
    }
}
